...
if nargin < 2
    Prior.LearningRateAlpha = 8; % beta distribution beta(1, 1) = uniform distribution
    Prior.LearningRateBeta = 12; % beta(8, 12) gives mean ~ 0.40, SD = 0.01 <- from simulation
    
    Prior.InverseTemperatureMean = 4; % normal distribution (can be neg. but fully)
    Prior.InverseTemperatureSigma = 1; % N(4, 1) <- from simulation
        
    Prior.ThresholdMean = 0.12; % normal distribution (can be neg. but fully)
    Prior.ThresholdSigma = 0.03; % N(0.12, 0.03) <- from simulation

    Prior.ForgettingRateAlpha = 2; % beta distribution beta(1, 1) = uniform distribution
    Prior.ForgettingRateBeta = 10; % beta(2, 10) gives mean ~ 0.167, SD ~ 0.01
...
try
    ...
    for iSession = 1:length(DataHolder)
        ...
        try
            Models{iSession} = Matching_SS_B_ChoiceForaging_Model(SessionData, Prior);
        catch
            Models{iSession} = [];
...
ThresholdMAPAxes = axes(AnalysisFigure, 'Position', [0.37    0.75    0.02    0.19]);
...
set(ThresholdMAPAxes,...
    ...
    'XTickLabel', '\theta',...
    'YAxisLocation', 'right')
ylabel(ThresholdMAPAxes, 'MAP estimate (a.u.)')

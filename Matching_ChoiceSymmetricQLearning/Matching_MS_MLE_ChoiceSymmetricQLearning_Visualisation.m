function AnalysisFigure = Matching_MS_MLE_ChoiceSymmetricQLearning_Visualisation(DataFolderPath)
% MS = MultiSession
...
try
    ...
    for iSession = 1:length(DataHolder)
        ...
        try
            Models{iSession} = Matching_SS_MLE_ChoiceSymmetricQLearning_Model(SessionData);
        catch
            Models{iSession} = [];
...
ChoiceForgettingRateMLEAxes = axes(AnalysisFigure, 'Position', [0.42    0.75    0.02    0.19]);
...
BiasMLEAxes = axes(AnalysisFigure, 'Position', [0.47    0.75    0.02    0.19]);
hold(BiasMLEAxes, 'on');
...
for iSession = 1:length(DataHolder)
    ...
    ChoiceForgettingRateMLEs(iSession) = EstimatedParameters(5);
    BiasMLEs(iSession) = EstimatedParameters(6);

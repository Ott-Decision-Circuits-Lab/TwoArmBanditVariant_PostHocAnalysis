function AnalysisFigure = Matching_MS_HB_ChoiceSymmetricQLearning_Visualisation(DataFolderPath, HyperPrior)
% MS = MultiSession
...
if nargin < 2
    % learning rate is beta distribution, hyperparameter = mu, kappa, where
    % alpha = mu * kappa, beta = (1-mu) * kappa, and
    % for beta(4, 12), mu = 1/4, kappa = 16
    % hyperprior for mu is beta distribution, controlled by MuAlpha and
    % MuBeta; for kappa is gamma distrbution, controlled by KappaAlpha and
    % KappsTheta
    HyperPrior.LearningRateMuAlpha = 4; % beta(4, 12) gives mean = 1/4, var = 0.011
    HyperPrior.LearningRateMuBeta = 12;
    HyperPrior.LearningRateKappaAlpha = 16; % gamma(16, 1) gives mean = 16, var = 16
    HyperPrior.LearningRateKappaTheta = 1;
    
    % inverse temperature is normal distribution, controlled by a
    % hyperprior of mean in normal distribution (MeanMu, MeanSigma), and of
    % 1./Sigma^2 (precision) in gamma (PrecisionAlpha, PrecisionTheta)
    HyperPrior.InverseTemperatureMeanMu = 8;
    HyperPrior.InverseTemperatureMeanSigma = 1;
    HyperPrior.InverseTemperaturePrecisionAlpha = 1; % G(1, 1) gives mean = 1, var = 1
    HyperPrior.InverseTemperaturePrecisionTheta = 1;
    
    % forgetting rate is beta distribution, controlled by a hyperprior of
    % mu in beta distribution (MuAlpha, MuBeta), and of kappa in gamma
    % distribution (KappaAlpha, KappaTheta), mu = 1/6, kappa = 12
    HyperPrior.ForgettingRateMuAlpha = 2; % beta(2, 10) gives mean = 1/6, var = 0.011
    HyperPrior.ForgettingRateMuBeta = 10;
    HyperPrior.ForgettingRateKappaAlpha = 12; % gamma(12, 1) gives mean = 12, var = 12
    HyperPrior.ForgettingRateKappaTheta = 1;
    
    % choice stickiness is normal distribution, controlled by a
    % hyperprior of mean in normal distribution (MeanMu, MeanSigma), and of
    % 1./Sigma^2 (precision) in gamma (PrecisionAlpha, PrecisionTheta)
    HyperPrior.ChoiceStickinessMeanMu = -1;
    HyperPrior.ChoiceStickinessMeanSigma = 1;
    HyperPrior.ChoiceStickinessPrecisionAlpha = 1; % G(1, 1) gives mean = 1, var = 1
    HyperPrior.ChoiceStickinessPrecisionTheta = 1;
    
    % choice forgetting rate is beta distribution, controlled by a hyperprior of
    % mu in beta distribution (MuAlpha, MuBeta), and of kappa in gamma
    ...
    HyperPrior.nChain = 4; % (count(nHyperPriors) + count(nPriors)) * 2
...
try
    ...
    try
        Model = Matching_MS_HB_ChoiceSymmetricQLearning_Model(DataHolder, HyperPrior);
        save(fullfile(DataFolderPath, strcat('\', AnalysisName, '.mat')), 'Model')
        disp('YOu aRE a bEAutIFul HUmaN BeiNG, saID anTOniO.')
    catch
        disp('Warning: Problem running model')
    end
end
...
for iSession = 1:length(DataHolder)
    ...
    %% Analysis across sessions
    Chain = vertcat(Model.SessionModel{iSession}.Chains{:});

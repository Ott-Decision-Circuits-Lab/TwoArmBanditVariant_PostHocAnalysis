function AnalysisFigure = TwoArmBanditVariant_Matching_PHighGLM(DataFile)
...
elseif ~strcmpi(SessionData.SettingsFile.GUIMeta.RiskType.String{SessionData.SettingsFile.GUI.RiskType}, 'BlockFixHolding') && ...
       ~strcmpi(SessionData.SettingsFile.GUIMeta.RiskType.String{SessionData.SettingsFile.GUI.RiskType}, 'BlockRandHolding')
...
FigureTitle = strcat(RatName, '_', SessionDateTime, '_Matching_PHighGLM');
...
try 
    ...
    X = [Choices, Rewards, Unrewards];
    X(isnan(X)) = 0;
    
    PHighGLM = fitglm(X, (RewardProbLeft > RewardProbRight)', 'distribution', 'binomial');

    % predict choices
    PredictedLeftChoiceProb = PHighGLM.Fitted.Response;
    LogOdds = PHighGLM.Fitted.LinearPredictor;   %logodds for both: left and right
...
if model
    ...
    ydataChoice = PHighGLM.Coefficients.Estimate(2:1+HistoryKernelSize);
    ydataReward = PHighGLM.Coefficients.Estimate(7:1+2*HistoryKernelSize);
    intercept = PHighGLM.Coefficients.Estimate(1);
    ...
    ResidualHistogram = plotResiduals(PHighGLM, 'Histogram');
    
    %% Residual Histogram
    ResidualLaggedAxes = axes(AnalysisFigure, 'Position', [0.23    0.36    0.15    0.11]);
    ResidualLagged = plotResiduals(PHighGLM, 'lagged', 'Marker', '.', 'MarkerSize', 1);
    
    %% Residual Histogram
    ResidualFittedAxes = axes(AnalysisFigure, 'Position', [0.04    0.19    0.15    0.11]);
    ResidualFitted = plotResiduals(PHighGLM, 'fitted', 'Marker', '.', 'MarkerSize', 1);
    
    %% Residual Histogram
    ResidualProbabilityAxes = axes(AnalysisFigure, 'Position', [0.23    0.19    0.15    0.11]);
    ResidualProbability = plotResiduals(PHighGLM, 'Probability', 'Marker', '.', 'MarkerSize', 1);
    ...
    if ~all(isnan(FeedbackWaitingTime))
        ...
        AbsModelResiduals = abs(PHighGLM.Residuals.Raw);
...
DataPath = strcat(DataFolder, RatName, '\bpod_session\', SessionDateTime, '\',...
                  RatName, '_TwoArmBanditVariant_', SessionDateTime, '_Matching_PHighGLM.png');

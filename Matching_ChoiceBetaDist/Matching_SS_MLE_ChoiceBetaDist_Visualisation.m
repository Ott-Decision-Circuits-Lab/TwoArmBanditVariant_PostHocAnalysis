function AnalysisFigure = Matching_SS_MLE_ChoiceBetaDist_Visualisation(DataFile, Model)
% SS = SingleSession
...
AnalysisName = 'Matching_SS_MLE_ChoiceBetaDist';
...
%% Beta-Dist learning model
% model
if nargin < 2
    try
        Model = Matching_SS_MLE_ChoiceBetaDist_Model(SessionData);
    catch
...
% extract parameters
AlphaLearningStepSize = EstimatedParameters(1); % \delta_\alpha
AlphaForgettingRate = EstimatedParameters(2); % \gamma_\alpha
RLearningRate = EstimatedParameters(3); % \alpha_R
RForgettingRate = EstimatedParameters(4); % \gamma_R
Bias = EstimatedParameters(5); %

% make prediction from Model (i.e. estimated parameters)
[~, Values] = ChoiceBetaDist(EstimatedParameters, nTrials, ChoiceLeft, Rewarded);

LeftValue = Values.LeftValue;
RightValue = Values.RightValue;
ChoiceLeftProb = Values.ChoiceLeftProb;
ChoiceLeftLogOdds = Values.ChoiceLeftLogOdds;

LeftAlpha = Values.LeftAlpha;
LeftNu = Values.LeftNu;
RightAlpha = Values.RightAlpha;
RightNu = Values.RightNu;
SignalDetectionThreshold = Values.SignalDetectionThreshold;

PredictedLeftChoiceProb = ChoiceLeftProb;
...
LogOdds = log(PredictedLeftChoiceProb ./ (1 - PredictedLeftChoiceProb));

% plot prediction
SmoothedPredictedChoiceLeft = smooth(PredictedChoice, BinWidth, 'moving','omitnan');
SmoothedPredictedChoicePlot = plot(BlockSwitchAxes, idxTrial, PredictedLeftChoiceProb * 100,...
                                   'LineStyle', '-',...
...
title(ModelParameterAxes, 'BetaDist-RL Fitted Parameters')

AlphaLearningStepSizeText = text(ModelParameterAxes, 0, 5,...
                                 strcat('\delta_\alpha = ', sprintf('%5.3f', AlphaLearningStepSize)),...
                                 'FontSize', 12,...
                                 'Interpreter', 'tex');

AlphaForgettingRateText = text(ModelParameterAxes, 0, 4,...
                               strcat('\gamma_\alpha = ', sprintf('%5.2f', AlphaForgettingRate)),...
                               'FontSize', 12,...
                               'Interpreter', 'tex');

RLearningRateText = text(ModelParameterAxes, 0, 3,...
                         strcat('\alpha_R = ', sprintf('%5.3f', RLearningRate)),...
                         'FontSize', 12,...
                         'Interpreter', 'tex');

RForgettingRateText = text(ModelParameterAxes, 0, 2,...
                           strcat('\gamma_R = ', sprintf('%5.3f', RForgettingRate)),...
                           'FontSize', 12,...
                           'Interpreter', 'tex');
...
if ~all(isnan(FeedbackWaitingTime))
    ...
    set(VevaiometricAxes,...
        'FontSize', 10,...
        'XLim', [min(ChoiceLeftLogOdds), max(ChoiceLeftLogOdds)],...
    ...
    [xdata, ydata, error] = BinData(ChoiceLeftLogOdds(HighTITrial), Exploited(HighTITrial), dvbin);
    vv = ~isnan(xdata) & ~isnan(ydata) & ~isnan(error);
    ...
    [xdata, ydata, error] = BinData(ChoiceLeftLogOdds(LowTITrial), Exploited(LowTITrial), dvbin);
    vv = ~isnan(xdata) & ~isnan(ydata) & ~isnan(error);
    ...
    HighTIGLM = fitglm(ChoiceLeftLogOdds(HighTITrial), Exploited(HighTITrial), 'Distribution', 'binomial');
    HighTIGLMPlot = plot(TISortedPsychometricAxes, xdata, predict(HighTIGLM, xdata)*100,...
                         'Marker', 'none',...
                         'Color', 'k',...
                         'LineWidth', 0.5);

    LowTIGLM = fitglm(ChoiceLeftLogOdds(LowTITrial), Exploited(LowTITrial), 'Distribution', 'binomial');
    ...
    TIRValue = SignalDetectionThreshold(NotBaited);

    X = [TIChosenValue', TIUnchosenValue', TIRValue'];
    ...
    set(ValueTIGLMAxes,...
        ...
        'XLim', [0, 5],...
        'XTick', [1, 2, 3, 4],...
        'XTickLabel', {'\beta_0', 'Q_{chosen}', 'Q_{unchosen}', 'R'},...
...
DataPath = strcat(DataFolder, RatName, '\bpod_session\', SessionDateTime, '\',...
                  RatName, '_TwoArmBanditVariant_', SessionDateTime, '_Matching_ChoiceBetaDist.png');

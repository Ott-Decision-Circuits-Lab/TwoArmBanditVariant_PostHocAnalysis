function AnalysisFigure = Matching_MS_MLE_ChoiceBetaDist_Visualisation(DataFolderPath)
% MS = MultiSession
...
AnalysisName = 'Matching_MS_MLE_ChoiceBetaDist';
...
try
    ...
    for iSession = 1:length(DataHolder)
        ...
        try
            Models{iSession} = Matching_SS_MLE_ChoiceBetaDist_Model(SessionData);
        catch
            Models{iSession} = [];
...
% MLE (maximum likelihood estimation) estimate of ChoiceBetaDist
AlphaLearningStepSizeMLEAxes = axes(AnalysisFigure, 'Position', [0.22    0.75    0.02    0.19]);
hold(AlphaLearningStepSizeMLEAxes, 'on');

AlphaLearningStepSizeMLEs = [];

set(AlphaLearningStepSizeMLEAxes,...
    'FontSize', 10,...
    'XLim', [-1, 1],...
    'XTick', 0,...
    'XTickLabel', '\delta_\alpha',...
    'YAxisLocation', 'right')

AlphaForgettingRateMLEAxes = axes(AnalysisFigure, 'Position', [0.27    0.75    0.02    0.19]);
hold(AlphaForgettingRateMLEAxes, 'on');

AlphaForgettingRateMLEs = [];

set(AlphaForgettingRateMLEAxes,...
    'FontSize', 10,...
    'XLim', [-1, 1],...
    'XTick', 0,...
    'XTickLabel', '\gamma_\alpha',...
    'YAxisLocation', 'right')

RLearningRateMLEAxes = axes(AnalysisFigure, 'Position', [0.32    0.75    0.02    0.19]);
hold(RLearningRateMLEAxes, 'on');

RLearningRateMLEs = [];

set(RLearningRateMLEAxes,...
    'FontSize', 10,...
    'XLim', [-1, 1],...
    'XTick', 0,...
    'XTickLabel', '\alpha_R',...
...
RForgettingRateMLEAxes = axes(AnalysisFigure, 'Position', [0.37    0.75    0.02    0.19]);
hold(RForgettingRateMLEAxes, 'on');

RForgettingRateMLEs = [];

set(RForgettingRateMLEAxes,...
    'FontSize', 10,...
    'XLim', [-1, 1],...
    'XTick', 0,...
    'XTickLabel', '\gamma',...
...
BiasMLEAxes = axes(AnalysisFigure, 'Position', [0.42    0.75    0.02    0.19]);
hold(BiasMLEAxes, 'on');
...
ylabel(BlockTransitionAxes, 'Left choice (%)')
...
set(ValueTIGLMAxes,...
    ...
    'XLim', [0, 5],...
    'XTick', [1, 2, 3, 4],...
    'XTickLabel', {'\beta_0', 'V_{chosen}', 'V_{unchosen}', 'V_R'},...
...
for iSession = 1:length(DataHolder)
    ...
    AlphaLearningStepSizeMLEs(iSession) = EstimatedParameters(1);
    AlphaForgettingRateMLEs(iSession) = EstimatedParameters(2);
    RLearningRateMLEs(iSession) = EstimatedParameters(3);
    RForgettingRateMLEs(iSession) = EstimatedParameters(4);
    BiasMLEs(iSession) = EstimatedParameters(5);
    
    MLEEstimates = [AlphaLearningStepSizeMLEs(iSession), AlphaForgettingRateMLEs(iSession),...
                    RForgettingRateMLEs(iSession), RLearningRateMLEs(iSession),...
                    BiasMLEs(iSession)];
    
    [NegLogDataLikelihood, Values] = ChoiceBetaDist(MLEEstimates, nTrials, ChoiceLeft, Rewarded);
    
    ChoiceLeftLogOdds = Values.ChoiceLeftLogOdds;
    PredictedChoiceLeftProb = Values.ChoiceLeftProb;

    PredictedChoice = double(PredictedChoiceLeftProb >= 0.5);
    PredictedChoice(isnan(ChoiceLeft)) = nan;
    

    AbsModelResiduals = abs(PredictedChoiceLeftProb - ChoiceLeft);
    Exploited = AbsModelResiduals <= 0.5;
    Explore = Exploited == 0;
    Exploit = Exploited == 1;
    ...
    Bin = linspace(-max(abs(ValidChoiceLeftLogOdds)), max(abs(ValidChoiceLeftLogOdds)), 11);
    [XData, YData, Error] = BinData(ValidChoiceLeftLogOdds, ValidExploited, Bin);
    ValidData = ~isnan(XData) & ~isnan(YData) & ~isnan(Error);
    
    ChoiceLeftPsychometricLine{iSession} = line(PsychometricAxes,...
    ...
    RValue = Values.SignalDetectionThreshold;
...
ValidChoiceLeftLogOdds = AllChoiceExploitLogOdds(ValidTrial);
...
ChoiceLeftPsychometricLine{iSession + 1} = line(PsychometricAxes,...
                                                   'xdata', XData(ValidData),...
                                                   'ydata', YData(ValidData) * 100,...
                                                   'LineStyle', '-',...
                                                   'LineWidth', 0.5,...
...
XData = zeros(size(AlphaLearningStepSizeMLEs));
LearningRateSwarmchart = swarmchart(AlphaLearningStepSizeMLEAxes,...
                                    XData,...
                                    AlphaLearningStepSizeMLEs,...
                                    'Marker', '.',...
...
LearningRateBoxchart = boxchart(AlphaLearningStepSizeMLEAxes, XData, AlphaLearningStepSizeMLEs);
...
XData = zeros(size(AlphaForgettingRateMLEs));
InverseTemperatureSwarmchart = swarmchart(AlphaForgettingRateMLEAxes,...
                                          XData,...
                                          AlphaForgettingRateMLEs,...
                                          'Marker', '.',...
                                          'MarkerEdgeColor', ColourPalette.Session,...
...
InverseTemperatureBoxchart = boxchart(AlphaForgettingRateMLEAxes, XData, AlphaForgettingRateMLEs);
...
XData = zeros(size(RLearningRateMLEs));
ForgettingRateSwarmchart = swarmchart(RLearningRateMLEAxes,...
                                      XData,...
                                      RLearningRateMLEs,...
                                      'Marker', '.',...
                                      'MarkerEdgeColor', ColourPalette.Session,...
...
ForgettingRateBoxchart = boxchart(RLearningRateMLEAxes, XData, RLearningRateMLEs);
...
XData = zeros(size(RForgettingRateMLEs));
ThresholdSwarmchart = swarmchart(RForgettingRateMLEAxes,...
                                 XData,...
                                 RForgettingRateMLEs,...
                                 'Marker', '.',...
                                 'MarkerEdgeColor', ColourPalette.Session,...
...
ThresholdBoxchart = boxchart(RForgettingRateMLEAxes, XData, RForgettingRateMLEs);

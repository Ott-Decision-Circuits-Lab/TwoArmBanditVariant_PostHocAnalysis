function AnalysisFigure = Matching_MS_MLE_ChoiceSymmetricQLearningNoMemory_Visualisation(DataFolderPath)
% MS = MultiSession
...
AnalysisName = 'Matching_MS_MLE_ChoiceSymmetricQLearningNoMemory';
...
try
    ...
    for iSession = 1:length(DataHolder)
        ...
        try
            Models{iSession} = Matching_SS_MLE_ChoiceSymmetricQLearningNoMemory_Model(SessionData);
        catch
            Models{iSession} = [];
...
BiasMLEAxes = axes(AnalysisFigure, 'Position', [0.37    0.75    0.02    0.19]);
hold(BiasMLEAxes, 'on');
...
set(ValueTIGLMAxes,...
    ...
    'XLim', [0, 4],...
    'XTick', [1, 2, 3],...
    'XTickLabel', {'\beta_0', 'V_{chosen}', 'V_{unchosen}'},...
...
for iSession = 1:length(DataHolder)
    ...
    BiasMLEs(iSession) = EstimatedParameters(4);
    
    MLEEstimates = [LearningRateMLEs(iSession), InverseTemperatureMLEs(iSession), ForgettingRateMLEs(iSession),...
                    BiasMLEs(iSession)];
    
    [NegLogDataLikelihood, Values] = ChoiceSymmetricQLearningNoMemory(MLEEstimates, nTrials, ChoiceLeft, Rewarded);
...
% Value-TI GLM Coeff.
X = [AllTIChosenValue', AllTIUnchosenValue'];

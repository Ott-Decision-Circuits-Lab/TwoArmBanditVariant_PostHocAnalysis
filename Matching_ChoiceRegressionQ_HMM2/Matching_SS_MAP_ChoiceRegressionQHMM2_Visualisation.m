function AnalysisFigure = Matching_SS_MAP_ChoiceRegressionQHMM2_Visualisation(DataFile, Model)
% SS = SingleSession
...
AnalysisName = 'Matching_SS_MAP_ChoiceRegressionQHMM2';
...
if nargin < 2
    try
        Model = Matching_SS_MLE_ChoiceRegressionQ_Model(SessionData);
    catch
...
ParameterStandardError = Model.ParameterStandardError;
...
% extract parameters
Bias = EstimatedParameters(1); % beta_0
QCoeff = EstimatedParameters(2); % gamma
Minus1ChoiceCoeff = EstimatedParameters(3); % beta_c1
Minus1RewardCoeff = EstimatedParameters(4); % beta_r1
Minus2ChoiceCoeff = EstimatedParameters(5); % beta_c2
Minus2RewardCoeff = EstimatedParameters(6); % beta_r2

% make prediction from Model (i.e. estimated parameters)
[~, Values] = ChoiceRegressionQ(EstimatedParameters, nTrials, ChoiceLeft, Rewarded);
...
title(ModelParameterAxes, 'Q-LM Fitted Parameters')

BiasText = text(ModelParameterAxes, 0, 6,...
                strcat('\beta_0 = ', sprintf('%5.3f±%5.3f', Bias, ParameterStandardError(1))),...
                'FontSize', 12,...
                'Interpreter', 'tex');

QCoeffText = text(ModelParameterAxes, 0, 5,...
                  strcat('\beta_{Q_{t-1}} = ', sprintf('%5.2f±%5.2f', QCoeff, ParameterStandardError(2))),...
                  'FontSize', 12,...
                  'Interpreter', 'tex');

Minus1ChoiceText = text(ModelParameterAxes, 0, 4,...
                        strcat('\beta_{c_{t-1}} = ', sprintf('%5.2f±%5.2f', Minus1ChoiceCoeff, ParameterStandardError(3))),...
                        'FontSize', 12,...
                        'Interpreter', 'tex');

Minus1RewardedText = text(ModelParameterAxes, 0, 3,...
                          strcat('\beta_{r_{t-1}} = ', sprintf('%5.2f±%5.2f', Minus1RewardCoeff, ParameterStandardError(4))),...
                          'FontSize', 12,...
                          'Interpreter', 'tex');

Minus2ChoiceText = text(ModelParameterAxes, 0, 2,...
                        strcat('\beta_{c_{t-2}} = ', sprintf('%5.2f±%5.2f', Minus2ChoiceCoeff, ParameterStandardError(5))),...
                        'FontSize', 12,...
                        'Interpreter', 'tex');

Minus2RewardedText = text(ModelParameterAxes, 0, 1,...
                          strcat('\beta_{r_{t-2}} = ', sprintf('%5.2f±%5.2f', Minus2RewardCoeff, ParameterStandardError(6))),...
                          'FontSize', 12,...
                          'Interpreter', 'tex');
...
if ~all(isnan(FeedbackWaitingTime))
    ...
    TIChosenLogOdds = ChosenLogOdds(NotBaited);

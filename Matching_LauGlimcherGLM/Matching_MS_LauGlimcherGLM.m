function Matching_MS_LauGlimcherGLM(DataFolderPath)
%{
MS = MultiSession
...
AnalysisName = 'Matching_MS_LauGlimcherGLM';
...
% Rewarded-Rewarded psychometric
RRPsychometricAxes = axes(AnalysisFigure, 'Position', [0.45, 0.75, 0.08, 0.21]);
hold(RRPsychometricAxes, 'on')

AllRRLogOdds = [];
AllRRChoiceLeft = [];

set(RRPsychometricAxes,...
    'TickDir', 'out' ,...
    'FontSize', 10,...
    'XLim', [-5 5],...
    'YLim', [0, 100],...
    'YTick', [0, 50, 100])
title(RRPsychometricAxes, 'Rew.-Rew.')
ylabel(RRPsychometricAxes, 'Left Choices (%)')

% Unrewarded-Rewarded psychometric
URPsychometricAxes = axes(AnalysisFigure, 'Position', [0.55, 0.75, 0.08, 0.21]);
hold(URPsychometricAxes, 'on')

AllURLogOdds = [];
AllURChoiceLeft = [];

set(URPsychometricAxes,...
    'TickDir', 'out' ,...
    'FontSize', 10,...
    'XLim', [-5 5],...
    'YLim', [0, 100],...
    'YTick', [0, 50, 100],...
    'YTickLabel', {})
title(URPsychometricAxes, 'Unrew.-Rew.')

% Rewarded-Unrewarded psychometric
RUPsychometricAxes = axes(AnalysisFigure, 'Position', [0.65, 0.75, 0.08, 0.21]);
hold(RUPsychometricAxes, 'on')

AllRULogOdds = [];
AllRUChoiceLeft = [];

set(RUPsychometricAxes,...
    'TickDir', 'out' ,...
    'FontSize', 10,...
    'XLim', [-5 5],...
    'YLim', [0, 100],...
    'YTick', [0, 50, 100],...
    'YTickLabel', {})
title(RUPsychometricAxes, 'Rew.-Unrew.')

% Unrewarded-Unrewarded psychometric
UUPsychometricAxes = axes(AnalysisFigure, 'Position', [0.75, 0.75, 0.08, 0.21]);
hold(UUPsychometricAxes, 'on')

AllUULogOdds = [];
AllUUChoiceLeft = [];

set(UUPsychometricAxes,...
    'TickDir', 'out' ,...
    'FontSize', 10,...
    'XLim', [-5 5],...
    'YLim', [0, 100],...
    'YTick', [0, 50, 100],...
    'YTickLabel', {})
title(UUPsychometricAxes, 'Unrew.-Unrew.')

% psychometric slope scatter plot
PsychometricSlopeAxes = axes(AnalysisFigure, 'Position', [0.87    0.75    0.12    0.12]);
hold(PsychometricSlopeAxes, 'on')

AllBeta = nan(4, length(DataHolder));

set(PsychometricSlopeAxes,...
    'TickDir', 'out' ,...
    'FontSize', 10,...
    'XLim', [0, 5])
ylabel(PsychometricSlopeAxes, 'Slope \beta')
...
for iSession = 1:length(DataHolder)
    ...
    % Rew-Rew Psychometric
    RRTrial = Rewarded(1:end-2) == 1 & Rewarded(2:end-1) == 1;
    RRTrial = [false, false, RRTrial];

    RRLogOdds = LogOdds(RRTrial)';
    RRChoiceLeft = ChoiceLeft(RRTrial);
    
    ValidIdx = ~isnan(RRLogOdds) & ~isnan(RRChoiceLeft);
    FitObject = fit(RRLogOdds(ValidIdx)', RRChoiceLeft(ValidIdx)', 'logistic',...
                    'StartPoint', [1, 1, 0],...
                    'Upper', [1, 100, Inf],...
                    'Lower', [1, 0, -Inf]);

    XData = -5:1:5;
    YData = feval(FitObject, XData);
    
    RRPsychometricLine{iSession} = line(RRPsychometricAxes,...
                                        'xdata', XData,...
                                        'ydata', YData * 100,...
                                        'LineStyle', '-',...
                                        'LineWidth', 0.5,...
    ...
    AllRRLogOdds = [AllRRLogOdds, RRLogOdds(ValidIdx)];
    AllRRChoiceLeft = [AllRRChoiceLeft, RRChoiceLeft(ValidIdx)];
    AllBeta(1, iSession) = FitObject.b;
    
    % Unrew-Rew Psychometric
    URTrial = Rewarded(1:end-2) ~= 1 & Rewarded(2:end-1) == 1;
    URTrial = [false, false, URTrial];

    URLogOdds = LogOdds(URTrial)';
    URChoiceLeft = ChoiceLeft(URTrial);
    
    ValidIdx = ~isnan(URLogOdds) & ~isnan(URChoiceLeft);
    FitObject = fit(URLogOdds(ValidIdx)', URChoiceLeft(ValidIdx)', 'logistic',...
                    'StartPoint', [1, 1, 0],...
                    'Upper', [1, 100, +Inf],...
                    'Lower', [1, 0, -Inf]);
    ...
    URPsychometricLine{iSession} = line(URPsychometricAxes,...
                                        'xdata', XData,...
                                        'ydata', YData * 100,...
                                        'LineStyle', '-',...
                                        'LineWidth', 0.5,...
    ...
    AllURLogOdds = [AllURLogOdds, URLogOdds(ValidIdx)];
    AllURChoiceLeft = [AllURChoiceLeft, URChoiceLeft(ValidIdx)];
    AllBeta(2, iSession) = FitObject.b;
    
    % Rew-Unrew Psychometric
    RUTrial = Rewarded(1:end-2) == 1 & Rewarded(2:end-1) ~= 1;
    RUTrial = [false, false, RUTrial];

    RULogOdds = LogOdds(RUTrial)';
    RUChoiceLeft = ChoiceLeft(RUTrial);
    
    ValidIdx = ~isnan(RULogOdds) & ~isnan(RUChoiceLeft);
    FitObject = fit(RULogOdds(ValidIdx)', RUChoiceLeft(ValidIdx)', 'logistic',...
                    'StartPoint', [1, 1, 0],...
                    'Upper', [1, 100, +Inf],...
                    'Lower', [1, 0, -Inf]);
    ...
    RUPsychometricLine{iSession} = line(RUPsychometricAxes,...
                                        'xdata', XData,...
                                        'ydata', YData * 100,...
                                        'LineStyle', '-',...
                                        'LineWidth', 0.5,...
    ...
    AllRULogOdds = [AllRULogOdds, RULogOdds(ValidIdx)];
    AllRUChoiceLeft = [AllRUChoiceLeft, RUChoiceLeft(ValidIdx)];
    AllBeta(3, iSession) = FitObject.b;

    % Unrew-Unrew Psychometric
    UUTrial = Rewarded(1:end-2) ~= 1 & Rewarded(2:end-1) ~= 1;
    UUTrial = [false, false, UUTrial];

    UULogOdds = LogOdds(UUTrial)';
    UUChoiceLeft = ChoiceLeft(UUTrial);
    
    ValidIdx = ~isnan(UULogOdds) & ~isnan(UUChoiceLeft);
    FitObject = fit(UULogOdds(ValidIdx)', UUChoiceLeft(ValidIdx)', 'logistic',...
                    'StartPoint', [1, 1, 0],...
                    'Upper', [1, 100, +Inf],...
                    'Lower', [1, 0, -Inf]);
    ...
    UUPsychometricLine{iSession} = line(UUPsychometricAxes,...
                                        'xdata', XData,...
                                        'ydata', YData * 100,...
                                        'LineStyle', '-',...
                                        'LineWidth', 0.5,...
    ...
    AllUULogOdds = [AllUULogOdds, UULogOdds(ValidIdx)];
    AllUUChoiceLeft = [AllUUChoiceLeft, UUChoiceLeft(ValidIdx)];
    AllBeta(4, iSession) = FitObject.b;
    ...
    [ExploreLogZScoreLineXData, ExploreLogZScoreLineYData] = Binvevaio(ExploringLogOdds, ExploringTISqrtZScore, 10);
    [ExploitLogZScoreLineXData, ExploitLogZScoreLineYData] = Binvevaio(ExploitingLogOdds, ExploitingTISqrtZScore, 10);
...
% Rew-Rew psychometric
FitObject = fit(AllRRLogOdds', AllRRChoiceLeft', 'logistic',...
                'StartPoint', [1, 1, 0],...
                'Upper', [1, +Inf, +Inf],...
                'Lower', [1, -Inf, -Inf]);
...
RRPsychometricLine{iSession + 1} = line(RRPsychometricAxes,...
                                        'xdata', XData,...
                                        'ydata', YData * 100,...
                                        'LineStyle', '-',...
                                        'LineWidth', 0.5,...
...
% Unrew-Rew psychometric
FitObject = fit(AllURLogOdds', AllURChoiceLeft', 'logistic',...
                'StartPoint', [1, 1, 0],...
                'Upper', [1, +Inf, +Inf],...
                'Lower', [1, -Inf, -Inf]);
...
URPsychometricLine{iSession + 1} = line(URPsychometricAxes,...
                                        'xdata', XData,...
                                        'ydata', YData * 100,...
                                        'LineStyle', '-',...
                                        'LineWidth', 0.5,...
...
% Rew-Unrew psychometric
FitObject = fit(AllRULogOdds', AllRUChoiceLeft', 'logistic',...
                'StartPoint', [1, 1, 0],...
                'Upper', [1, +Inf, +Inf],...
                'Lower', [1, -Inf, -Inf]);
...
RUPsychometricLine{iSession + 1} = line(RUPsychometricAxes,...
                                        'xdata', XData,...
                                        'ydata', YData * 100,...
                                        'LineStyle', '-',...
                                        'LineWidth', 0.5,...
                                        'Color', ColourPalette.Pooled);
% Unrew-Unrew psychometric
FitObject = fit(AllUULogOdds', AllUUChoiceLeft', 'logistic',...
                'StartPoint', [1, 1, 0],...
                'Upper', [1, +Inf, +Inf],...
                'Lower', [1, -Inf, -Inf]);
...
UUPsychometricLine{iSession + 1} = line(UUPsychometricAxes,...
                                        'xdata', XData,...
                                        'ydata', YData * 100,...
                                        'LineStyle', '-',...
                                        'LineWidth', 0.5,...
...
% slope-beta scatter
Index = repmat((1:4)', 1, length(DataHolder));

BetaSwarmchart = swarmchart(PsychometricSlopeAxes,...
                            Index - 0.1,...
                            AllBeta,...
                            'Marker', '.',...
...
BetaBoxchart = boxchart(PsychometricSlopeAxes,...
                        reshape(Index, 1, []) + 0.1,...
                        reshape(AllBeta, 1, []));
...
set(PsychometricSlopeAxes,...
    ...
    'XLim', [0, 5],...
    'XTick', 1:4,...
    'XTickLabel', {'Rew-Rew', 'Unrew-Rew', 'Rew-Unrew', 'Unrew-Unrew'},...
    'XAxisLocation', 'top',...
    'XTickLabelRotation', 90,...
    'YLim', [0, 3])
ylabel(PsychometricSlopeAxes, '\beta (a.u.)')
...
% Vevaiometric z-score
[ExploreLogZScoreLineXData, ExploreLogZScoreLineYData] = Binvevaio(AllExploringLogOdds, AllExploringTISqrtZScore, 10);
[ExploitLogZScoreLineXData, ExploitLogZScoreLineYData] = Binvevaio(AllExploitingLogOdds, AllExploitingTISqrtZScore, 10);
...
Block4To5Explore = mean(Block4To5Explore, 'omitnan');
Block4To5ExploreLine = line(BlockTransitionAxes,...
                            'xdata', -15:50,...
                            'ydata', movmean(Block4To5Explore, [5, 0]) * 100,...
                            'LineStyle', '-',...
                            'Marker', 'none',...
                            'Color', 0.6 * [1 1 1]);

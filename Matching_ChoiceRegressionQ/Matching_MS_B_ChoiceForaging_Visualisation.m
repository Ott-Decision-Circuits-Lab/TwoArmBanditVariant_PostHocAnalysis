...
if nargin < 2
    ...
    Prior.BiasSigma = 5; % N(0, 5)
    
    Prior.QCoeffAlpha = 10; % beta distribution beta(1, 1) = uniform distribution
    Prior.QCoeffBeta = 2; % beta(10, 2) gives mean ~ 0.833, SD ~ 0.01
        
    Prior.Minus1ChoiceMean = 0; % normal distribution
    Prior.Minus1ChoiceSigma = 5; % N(0, 5)
    
    Prior.Minus1RewardMean = 0; % normal distribution
    Prior.Minus1RewardSigma = 5; % N(0, 5)
    
    Prior.Minus2ChoiceMean = 0; % normal distribution
    Prior.Minus2ChoiceSigma = 5; % N(0, 5)
    
    Prior.Minus2RewardMean = 0; % normal distribution
    Prior.Minus2RewardSigma = 5; % N(0, 5)
...
% ChoiceForaging
BiasMAPAxes = axes(AnalysisFigure, 'Position', [0.22    0.75    0.02    0.19]);
hold(BiasMAPAxes, 'on');
...
QCoeffMAPAxes = axes(AnalysisFigure, 'Position', [0.27    0.75    0.02    0.19]);
hold(QCoeffMAPAxes, 'on');

QCoeffMAPs = nan(1, nSessions);
...
Minus1ChoiceMAPAxes = axes(AnalysisFigure, 'Position', [0.32    0.75    0.02    0.19]);
hold(Minus1ChoiceMAPAxes, 'on');

Minus1ChoiceMAPs = nan(1, nSessions);

set(Minus1ChoiceMAPAxes,...
    'FontSize', 10,...
    'XLim', [-1, 1],...
    'XTick', 0,...
    'XTickLabel', '\beta_c_{t-1}',...
    'YAxisLocation', 'right')

Minus1RewardMAPAxes = axes(AnalysisFigure, 'Position', [0.37    0.75    0.02    0.19]);
hold(Minus1RewardMAPAxes, 'on');

Minus1RewardMAPs = nan(1, nSessions);

set(Minus1RewardMAPAxes,...
    'FontSize', 10,...
    'XLim', [-1, 1],...
    'XTick', 0,...
    'XTickLabel', '\beta_r_{t-1}',...
    'YAxisLocation', 'right')

Minus2ChoiceMAPAxes = axes(AnalysisFigure, 'Position', [0.42    0.75    0.02    0.19]);
hold(Minus2ChoiceMAPAxes, 'on');

Minus2ChoiceMAPs = nan(1, nSessions);

set(Minus2ChoiceMAPAxes,...
    'FontSize', 10,...
    'XLim', [-1, 1],...
    'XTick', 0,...
    'XTickLabel', '\beta_c_{t-2}',...
    'YAxisLocation', 'right')

Minus2RewardMAPAxes = axes(AnalysisFigure, 'Position', [0.47    0.75    0.02    0.19]);
hold(Minus2RewardMAPAxes, 'on');

Minus2RewardMAPs = nan(1, nSessions);

set(Minus2RewardMAPAxes,...
    'FontSize', 10,...
    'XLim', [-1, 1],...
    'XTick', 0,...
    'XTickLabel', '\beta_r_{t-2}',...
    'YAxisLocation', 'right')
ylabel(Minus2RewardMAPAxes, 'MAP estimate (a.u.)')
...
%{
% Value-TI GLM Coeff.
ValueTIGLMAxes = axes(AnalysisFigure, 'Position', [0.58, 0.78, 0.10, 0.16]);
hold(ValueTIGLMAxes, 'on')

ValueTIGLMCoeff = nan(nSessions, 4);
...
set(ValueTIGLMAxes,...
    ...
    'XLim', [0, 5],...
    'XTick', [1, 2, 3, 4],...
    'XTickLabel', {'\beta_0', 'V_{chosen}', 'V_{unchosen}', 'm_{chosen}'},...
...
ValueTISqrtZScoreGLMAxes = axes(AnalysisFigure, 'Position', [0.70, 0.78, 0.10, 0.16]);
hold(ValueTISqrtZScoreGLMAxes, 'on')


ValueTISqrtZScoreGLMCoeff = nan(nSessions, 4);
...
% Chosen LogOdds & TotalValue (CLOTV)-TI GLM Coeff.
CLOTVTIGLMAxes = axes(AnalysisFigure, 'Position', [0.82, 0.78, 0.07, 0.16]);
hold(CLOTVTIGLMAxes, 'on')

CLOTVTIGLMCoeff = nan(nSessions, 3);
CLOTVTISqrtZScoreGLMCoeff = nan(nSessions, 3);
...
set(CLOTVTIGLMAxes,...
    ...
    'XLim', [0, 4],...
    'XTick', [1, 2, 3],...
    'XTickLabel', {'\beta_0', 'Q_{chosen}', 'sum(V)'},...
    'XTickLabelRotation', 90,...
    'FontSize', 10);
title(CLOTVTIGLMAxes, 'CLOTV-TI');

% ChosenLogOdds & TotalValue (CLOTV)-sqrt(TI) z-score GLM Coeff.
CLOTVTISqrtZScoreGLMAxes = axes(AnalysisFigure, 'Position', [0.91, 0.78, 0.07, 0.16]);
hold(CLOTVTISqrtZScoreGLMAxes, 'on')

set(CLOTVTISqrtZScoreGLMAxes,...
    'TickDir', 'out',...
    'XLim', [0, 4],...
    'XTick', [1, 2, 3],...
    'XTickLabel', {'\beta_0', 'Q_{chosen}', 'sum(V)'},...
    'XTickLabelRotation', 90,...
    'FontSize', 10);
title(CLOTVTISqrtZScoreGLMAxes, 'CLOTV-sqrt(TI) (z)');
%}
disp('YOu aRE a bEAutIFul HUmaN BeiNG, saID anTOniO.')
...
for iSession = 1:length(DataHolder)
    ...
    QCoeffMAPs(iSession) = Values(ProbDensity == max(ProbDensity));
    
    [ProbDensity, Values] = ksdensity(Chain(:, 4));
    Minus1ChoiceMAPs(iSession) = Values(ProbDensity == max(ProbDensity));
    
    [ProbDensity, Values] = ksdensity(Chain(:, 3));
    Minus1RewardMAPs(iSession) = Values(ProbDensity == max(ProbDensity));
    
    MAPEstimates = [BiasMAPs(iSession), QCoeffMAPs(iSession), Minus1ChoiceMAPs(iSession),...
                    Minus1RewardMAPs(iSession)];
    ...
    X = [ChosenValue', UnchosenValue', ChosenMemory'];
    ValueTIGLM = fitglm(X(NotBaited, :), FeedbackWaitingTime(NotBaited));

    ValueTIGLMCoeff(iSession, :) = ValueTIGLM.Coefficients.Estimate;
    ...
    ValueTISqrtZScoreGLM = fitglm(X(NotBaited, :), FeedbackWaitingTimeSqrtZScore(NotBaited));
    ValueTISqrtZScoreGLMCoeff(iSession, :) = ValueTISqrtZScoreGLM.Coefficients.Estimate;
    ...
    % LogOdds & TotalValue (LOTV) -TI GLM Coeff. & sqrt(TI) (z)
    ChosenLogOdds = LogOdds .* ChoiceLeft - LogOdds .* (1 - ChoiceLeft);
    TotalValue = ChosenValue + UnchosenValue;
    
    X = [ChosenLogOdds', TotalValue'];
    CLOTVTIGLM = fitglm(X(NotBaited, :), FeedbackWaitingTime(NotBaited));
    CLOTVTIGLMCoeff(iSession, :) = CLOTVTIGLM.Coefficients.Estimate;

    CLOTVTISqrtZScoreGLM = fitglm(X(NotBaited, :), FeedbackWaitingTimeSqrtZScore(NotBaited));
    CLOTVTISqrtZScoreGLMCoeff(iSession, :) = CLOTVTISqrtZScoreGLM.Coefficients.Estimate;
...
LearningRateSwarmchart = swarmchart(BiasMAPAxes,...
                                    XData,...
                                    BiasMAPs,...
                                    'Marker', '.',...
...
LearningRateBoxchart = boxchart(BiasMAPAxes, XData, BiasMAPs);
...
XData = zeros(size(QCoeffMAPs));
InverseTemperatureSwarmchart = swarmchart(QCoeffMAPAxes,...
                                          XData,...
                                          QCoeffMAPs,...
                                          'Marker', '.',...
                                          'MarkerEdgeColor', ColourPalette.Session,...
...
InverseTemperatureBoxchart = boxchart(QCoeffMAPAxes, XData, QCoeffMAPs);
...
XData = zeros(size(Minus1ChoiceMAPs));
ForgettingRateSwarmchart = swarmchart(Minus1ChoiceMAPAxes,...
                                      XData,...
                                      Minus1ChoiceMAPs,...
                                      'Marker', '.',...
                                      'MarkerEdgeColor', ColourPalette.Session,...
...
ForgettingRateBoxchart = boxchart(Minus1ChoiceMAPAxes, XData, Minus1ChoiceMAPs);
...
XData = zeros(size(Minus1RewardMAPs));
ThresholdSwarmchart = swarmchart(Minus1RewardMAPAxes,...
                                 XData,...
                                 Minus1RewardMAPs,...
                                 'Marker', '.',...
                                 'MarkerEdgeColor', ColourPalette.Session,...
...
ThresholdBoxchart = boxchart(Minus1RewardMAPAxes, XData, Minus1RewardMAPs);
...
X = reshape(X, 1, []);
ValueTIGLMCoeff = reshape(ValueTIGLMCoeff, 1, []);
SessionValueTIGLMSwarmchart = swarmchart(ValueTIGLMAxes,...
                                         X,...
                                         ValueTIGLMCoeff,...
                                         'Marker', '.',...
                                         'MarkerEdgeColor', ColourPalette.Session,...
...
ValueTISqrtZScoreGLMCoeff = reshape(ValueTISqrtZScoreGLMCoeff, 1, []);
SessionValueTISqrtZScoreGLMSwarmchart = swarmchart(ValueTISqrtZScoreGLMAxes,...
                                                   X,...
                                                   ValueTISqrtZScoreGLMCoeff,...
                                                   'Marker', '.',...
                                                   'MarkerEdgeColor', ColourPalette.Session,...
                                                   'XJitter', 'density',...
                                                   'XJitterWidth', 0.4);
...
ValueTICoeffBar = bar(ValueTIGLMAxes, ValueTIGLM.Coefficients.Estimate,...
                      'FaceColor', 'none');
...
ValueTISqrtZScoreCoeffBar = bar(ValueTISqrtZScoreGLMAxes, ValueTISqrtZScoreGLM.Coefficients.Estimate,...
                                'FaceColor', 'none');
...
% ChosenLogOdds & TotalValue(CLOTV) - TI GLM
X = repmat(1:3, nSessions, 1);
X = reshape(X, 1, []);
CLOTVTIGLMCoeff = reshape(CLOTVTIGLMCoeff, 1, []);
SessionCLOTVTIGLMSwarmchart = swarmchart(CLOTVTIGLMAxes,...
                                         X,...
                                         CLOTVTIGLMCoeff,...
                                         'Marker', '.',...
                                         'MarkerEdgeColor', ColourPalette.Session,...
...
CLOTVTISqrtZScoreGLMCoeff = reshape(CLOTVTISqrtZScoreGLMCoeff, 1, []);
SessionCLOTVTISqrtZScoreGLMSwarmchart = swarmchart(CLOTVTISqrtZScoreGLMAxes,...
                                                   X,...
                                                   CLOTVTISqrtZScoreGLMCoeff,...
                                                   'Marker', '.',...
                                                   'MarkerEdgeColor', ColourPalette.Session,...
                                                   'XJitter', 'density',...
                                                   'XJitterWidth', 0.4);

X = [AllTIChosenLogOdds', AllTITotalValue'];
CLOTVTIGLM = fitglm(X, AllTI);

CLOTVTICoeffBar = bar(CLOTVTIGLMAxes, CLOTVTIGLM.Coefficients.Estimate,...
                      'FaceColor', 'none');
CLOTVTICoeffErrorbar = errorbar(CLOTVTIGLMAxes,...
                                CLOTVTIGLM.Coefficients.Estimate,...
                                CLOTVTIGLM.Coefficients.SE,...
                                'LineStyle', 'none',...
                                'Color', 'k');

SignificantLevel = 0.05;
SignificanceIdx = find(CLOTVTIGLM.Coefficients.pValue < SignificantLevel);
CLOTVTISignificantLine = line(CLOTVTIGLMAxes,...
                              SignificanceIdx,...
                              zeros(size(SignificanceIdx)),...
                              'LineStyle', 'none',...
                              'Color', 'k',...
...
CLOTVTISqrtZScoreGLM = fitglm(X, AllTISqrtZScore);

CLOTVTISqrtZScoreCoeffBar = bar(CLOTVTISqrtZScoreGLMAxes, CLOTVTISqrtZScoreGLM.Coefficients.Estimate,...
                                'FaceColor', 'none');
CLOTVTISqrtZScoreCoeffErrorbar = errorbar(CLOTVTISqrtZScoreGLMAxes,...
                                          CLOTVTISqrtZScoreGLM.Coefficients.Estimate,...
                                          CLOTVTISqrtZScoreGLM.Coefficients.SE,...
                                          'LineStyle', 'none',...
                                          'Color', 'k');

SignificantLevel = 0.05;
SignificanceIdx = find(CLOTVTISqrtZScoreGLM.Coefficients.pValue < SignificantLevel);
CLOTVTISqrtZScoreSignificantLine = line(CLOTVTISqrtZScoreGLMAxes,...
                                        SignificanceIdx,...
                                        zeros(size(SignificanceIdx)),...
                                        'LineStyle', 'none',...
                                        'Color', 'k',...

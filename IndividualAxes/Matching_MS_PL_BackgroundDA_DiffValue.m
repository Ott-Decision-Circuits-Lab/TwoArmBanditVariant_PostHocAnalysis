function AnalysisFigure = Matching_MS_PL_BackgroundDA_DiffValue(DataFolderPath, PhotometryDatasetFilePath, ModelsFilePath, FigureSize, AxeSize)
...
AnalysisName = 'Matching_MS_PL_BackgroundDA_DiffValue';
...
BackgroundDADiffValueAxes = axes(AnalysisFigure,...
                                  'Position', AxeSize,...
                                  'Units', 'inches',...
                                  'Color', 'none');
set(BackgroundDADiffValueAxes,...
    'Position', AxeSize,...
    'Units', 'inches');
hold(BackgroundDADiffValueAxes, 'on');

DiffValue = LeftValue - RightValue;

ValidIdx = ~isnan(ChosenValue) & ~isnan(dFEstimation);

BackgroundDADiffValueScatter = scatter(BackgroundDADiffValueAxes, DiffValue(ValidIdx), dFEstimation(ValidIdx),...
                                       'Marker', '.',...
                                       'MarkerEdgeColor', 'k',...
                                       'SizeData', 1);

[RValue, pValue] = corrcoef(DiffValue(ValidIdx), dFEstimation(ValidIdx));
...
BackgroundDADiffValueStatText = text(BackgroundDADiffValueAxes, 0.1, 0.2,...
...
X = [ones(1, sum(ValidIdx)); DiffValue(ValidIdx)]';
b = X \ dFEstimation(ValidIdx)';

XData = [-0.6, 0.6];
YData = XData * b(2) + b(1);

BackgroundDADiffValueRegressionLine = line(BackgroundDADiffValueAxes, XData, YData,...
                                           'Color', 'k',...
                                           'LineWidth', 1);
...
set(BackgroundDADiffValueAxes,...
    ...
    'YTick', [-20, 0, 20],...
    'FontSize', 12);
xlabel(BackgroundDADiffValueAxes, 'V_{L-R} (a.u.)')
ylabel(BackgroundDADiffValueAxes, sprintf('Baseline dopamine (dF/F)'))

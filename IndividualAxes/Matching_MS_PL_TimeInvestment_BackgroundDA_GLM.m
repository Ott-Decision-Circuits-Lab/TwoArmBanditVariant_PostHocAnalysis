function AnalysisFigure = Matching_MS_PL_TimeInvestment_BackgroundDA_GLM(DataFolderPath, PhotometryDatasetFilePath, ModelsFilePath, FigureSize, AxeSize)
...
AnalysisName = 'Matching_MS_PL_TimeInvestment_BackgroundDA_GLM';
...
for iSession = 1:length(PhotometryDataHolder)
    ...
    Model = PhotometryModels{iSession};
...
XData = [ChosenLogOdds', dFEstimationZScore'];
YData = TimeInvestmentZScore;
ValidIdx = ~isnan(ChosenLogOdds) & ~isoutlier(dFEstimationZScore) & ~isnan(YData);
...
for iCoeff = 1:length(pValues)
    ...
    text(GLMCoeffAxes, iCoeff, 0.3, pValueSymbol,...
         'FontSize', 24,...
         'HorizontalAlignment', 'center')
end

Labels = {"Intercept", "Q_{chosen}", "dF/F (z)"};

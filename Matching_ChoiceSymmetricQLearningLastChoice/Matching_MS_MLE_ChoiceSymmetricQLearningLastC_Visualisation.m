function AnalysisFigure = Matching_MS_MLE_ChoiceSymmetricQLearningLastC_Visualisation(DataFolderPath)
% MS = MultiSession
...
AnalysisName = 'Matching_MS_MLE_ChoiceSymmetricQLearningLastC';
...
try
    ...
    for iSession = 1:length(DataHolder)
        ...
        try
            Models{iSession} = Matching_SS_MLE_ChoiceSymmetricQLearningLastC_Model(SessionData);
        catch
            Models{iSession} = [];
...
InverseTemperatureMLEAxes = axes(AnalysisFigure, 'Position', [0.27    0.75    0.02    0.19]);
...
ForgettingRateMLEAxes = axes(AnalysisFigure, 'Position', [0.32    0.75    0.02    0.19]);
...
ChoiceStickinessMLEAxes = axes(AnalysisFigure, 'Position', [0.37    0.75    0.02    0.19]);
...
set(BiasMLEAxes,...
    ...
    'XTickLabel', 'bias',...
    'YAxisLocation', 'right')
ylabel(BiasMLEAxes, 'MLE estimate (a.u.)')
...
for iSession = 1:length(DataHolder)
    ...
    LearningRateMLEs(iSession) = EstimatedParameters(1);
    InverseTemperatureMLEs(iSession) = EstimatedParameters(2);
    ForgettingRateMLEs(iSession) = EstimatedParameters(3);
    ChoiceStickinessMLEs(iSession) = EstimatedParameters(4);
    ...
    MLEEstimates = [LearningRateMLEs(iSession), InverseTemperatureMLEs(iSession), ForgettingRateMLEs(iSession),...
                    ChoiceStickinessMLEs(iSession), BiasMLEs(iSession)];
    
    [NegLogDataLikelihood, Values] = ChoiceSymmetricQLearningLastC(MLEEstimates, nTrials, ChoiceLeft, Rewarded);

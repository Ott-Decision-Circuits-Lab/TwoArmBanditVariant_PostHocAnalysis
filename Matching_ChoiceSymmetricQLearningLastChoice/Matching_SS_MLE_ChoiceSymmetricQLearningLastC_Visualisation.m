function AnalysisFigure = Matching_SS_MLE_ChoiceSymmetricQLearningLastC_Visualisation(DataFile, Model)
% SS = SingleSession
...
AnalysisName = 'Matching_SS_MLE_ChoiceSymmetricQLearningLastC';
...
if nargin < 2
    try
        Model = Matching_SS_MLE_ChoiceSymmetricQLearningLastC_Model(SessionData);
    catch
...
[~, Values] = ChoiceSymmetricQLearningNoMemory(EstimatedParameters, nTrials, ChoiceLeft, Rewarded);
...
LearningRateText = text(ModelParameterAxes, 0, 5,...
                        strcat('\alpha = ', sprintf('%5.3f±%5.3f', LearningRate, ParameterStandardError(1))),...
                        'FontSize', 12,...
                        'Interpreter', 'tex');

InverseTemperatureText = text(ModelParameterAxes, 0, 4,...
                              strcat('\beta = ', sprintf('%5.2f±%5.2f', InverseTemperature, ParameterStandardError(2))),...
                              'FontSize', 12,...
                              'Interpreter', 'tex');

ForgettingRateText = text(ModelParameterAxes, 0, 3,...
                          strcat('\gamma = ', sprintf('%5.3f±%5.3f', ForgettingRate, ParameterStandardError(3))),...
                          'FontSize', 12,...
                          'Interpreter', 'tex');

ChoiceStickinessText = text(ModelParameterAxes, 0, 2,...
                            strcat('\phi = ', sprintf('%5.3f±%5.3f', ChoiceStickiness, ParameterStandardError(4))),...
                            'FontSize', 12,...
                            'Interpreter', 'tex');

BiasText = text(ModelParameterAxes, 0, 1,...
                strcat('bias = ', sprintf('%5.3f±%5.3f', Bias, ParameterStandardError(5))),...
                'FontSize', 12,...
                'Interpreter', 'tex');
...
if ~all(isnan(FeedbackWaitingTime))
    ...
    X = [TIChosenValue', TIUnchosenValue'];
...
DataPath = strcat(DataFolder, RatName, '\bpod_session\', SessionDateTime, '\',...
                  RatName, '_TwoArmBanditVariant_', SessionDateTime, '_Matching_ChoiceSymmetricQLearningLastC.png');
